...
func (ts *Test) one(cmd any, expectedServers int, retry bool) int {
	...
	t0 := time.Now()
	starts := 0
	log.Printf("Starting agreement check for command: %.8s, expecting %d servers", textcmd, expectedServers)
	...
	for time.Since(t0).Seconds() < 10 && ts.checkFinished() == false {
		...
		for range ts.srvs {
			...
			if rf != nil {
				log.Printf("Attempting to start command %.8s on peer %d", textcmd, starts)
				index1, _, ok := rf.Start(cmd)
				if ok {
					index = index1
					log.Printf("Command %.8s successfully started on peer %d, log index %d", textcmd, starts, index)
					break
				}
			}
		}
		...
		if index != -1 {
			...
			// submitted our command; wait a while for agreement.
			t1 := time.Now()
			log.Printf("Waiting for agreement on command %.8s, index %d", textcmd, index)

			for time.Since(t1).Seconds() < 2 {
				nd, cmd1 := ts.nCommitted(index)
				log.Printf("Checking agreement: %d nodes committed, expecting at least %d", nd, expectedServers)
				if nd > 0 && nd >= expectedServers {
					// committed
					if cmd1 == cmd {
						// and it was the command we submitted.
						desp := fmt.Sprintf("agreement of %.8s reached", textcmd)
						tester.AnnotateCheckerSuccess(desp, "OK")
						log.Printf("Agreement reached for command %.8s", textcmd)
						return index
					}
				}
				time.Sleep(20 * time.Millisecond)
			...
			log.Printf("Leader not found, retrying...")
			time.Sleep(50 * time.Millisecond)
		}
	}
